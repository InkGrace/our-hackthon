...
这是单页应用（SPA）在 GitHub Pages 上的常见问题。项目已自动处理此问题。

**解决方案**：

- 项目已配置 Vite 插件，构建时会自动将 `index.html` 复制为 `404.html`
- GitHub Pages 会自动使用 `404.html` 处理所有找不到的页面
- `404.html` 与 `index.html` 内容相同，Vue Router 可以正常处理路由

如果仍然遇到问题：

1. 检查构建输出中是否包含 `dist/404.html`
2. 确认 `dist/404.html` 与 `dist/index.html` 内容相同
3. 清除浏览器缓存后重试
